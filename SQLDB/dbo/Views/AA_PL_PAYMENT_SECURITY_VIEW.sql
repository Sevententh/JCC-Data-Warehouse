create view AA_PL_PAYMENT_SECURITY_VIEW
/*
** Written:			5/4/2004 RL
** Last Amended: 
** Comments: 		Returns Security Profile settings for Purchase Payments
*/
as
select
...
cast (substring(UP_TRAN_OPTIONS2, 7, 1) as tinyint) as REMEMBER_SUPPLIER_DETAILS,
...
as ACCOUNT_NOTES,
(case 
   when substring(UP_TRAN_OPTIONS2, 12, 1) = 'Y' then cast(1 as tinyint)
   else cast(0 as tinyint)
   end)
as AGED_CREDITOR
