CREATE VIEW AA_NL_BS_YTD_NOMINAL_VIEW
AS
/*
** 
** Intended to be called by AA_NL_BALANCE_SHEET_NOMINAL_S with AA_NL_BALANCE_SHEET_NOMINAL_VIEW
** The main search fields are in CREATE VIEW AA_NL_BS_YTD_NOMINAL_VIEW along with the other financial fields.
...
** Last Amended: 10/10/2006 SB, 20/01/2006 SB
*/
select
   ...
   ,dbo.AA_VALUE_DPS_F(isnull(BUD_YTD_L,cast(0 as decimal(24,8))) * case NMAJORCODE when 1 then -1 when 3 then -1 when 10 then -1 when 11 then -1 when 12 then -1 else 1 end) BUD_YTD_L
   ,isnull(BV.B_ACCOUNT_CODE, N.NCODE) B_ACCOUNT_CODE
   ,isnull(max(BV.B_REVISION_NUMBER),0) REV_NO
from
   AA_NL_BALANCE_SHEET_NOMINAL_VIEW N 
   ...
   left join (select 
         ...
         ,N.CATEGORY
         --£ Last Year Budget YTD
         ,B_VALUE * case NMAJORCODE when 1 then -1 when 3 then -1 when 10 then -1 when 11 then -1 when 12 then -1 else 1 end BUD_YTD_L
         ,isnull(BV.B_ACCOUNT_CODE, N.NCODE) B_ACCOUNT_CODE
