create view AA_CST_CC_TURNOVER_LAST_YEAR_VIEW
/*
...
from 
   (  select PERIOD
      ...
      , CV.[PRIMARY]
      , cast(case
            when PERIOD = 1  then CC.CC_COST_L_1       
            when PERIOD = 2  then CC.CC_COST_L_2   
            when PERIOD = 3  then CC.CC_COST_L_3   
            when PERIOD = 4  then CC.CC_COST_L_4   
            when PERIOD = 5  then CC.CC_COST_L_5   
            when PERIOD = 6  then CC.CC_COST_L_6   
            when PERIOD = 7  then CC.CC_COST_L_7   
            when PERIOD = 8  then CC.CC_COST_L_8   
            when PERIOD = 9  then CC.CC_COST_L_9   
            when PERIOD = 10 then CC.CC_COST_L_10  
            when PERIOD = 11 then CC.CC_COST_L_11  
            when PERIOD = 12 then CC.CC_COST_L_12  
            when PERIOD = 13 then CC.CC_COST_L_13  
   ...
   left join CST_SALES_TURNOVERS T
      on C.CH_PRIMARY = T.CTO_PROJECT_LINK and C.CC_PRIMARY = T.CTO_CENTRE_LINK
         and ISNULL(T.CTO_YEAR_LINK,(SELECT YEAR_PRIMARY FROM SYS_YEAR WHERE YEAR_COSTING_CODE = 'L')) 
                                  = (SELECT YEAR_PRIMARY FROM SYS_YEAR WHERE YEAR_COSTING_CODE = 'L')
