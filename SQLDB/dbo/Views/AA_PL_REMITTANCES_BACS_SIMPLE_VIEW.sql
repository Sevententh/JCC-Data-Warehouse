create view AA_PL_REMITTANCES_BACS_SIMPLE_VIEW
/*
** Returns a table to be used specifically with AA_PL_REMITTANCES_BACS_LIST_S
** Written: 17/02/2005 SH
** Last Amended: 08/04/2005 SRB, 01/07/2005 SH, 25/08/2005; 01/11/05 SRB;
**
*/
as
	select T.PT_PRIMARY as [PRIMARY],
		...
		case when T.PT_TRANTYPE in ( 'PAY', 'ADR', 'CRN' ) then PT_PAYLIST_VALUE * -1
			else PT_PAYLIST_VALUE
		...
		case when isnull( D2.PL_PAYMENT_STAGES, 0 ) > T.PT_PAY_STAGE_STATUS then 0
			else 1
		end as APPROVED_TRANSACTION,
		case when T.PT_YEAR = 'N' or ( T.PT_YEAR = 'C' and T.PT_PERIODNUMBER > D.PURCHASE_PERIOD ) then 1
			else 0
		end as FUTURE_PERIOD_POSTING,
		isnull( S.SU_BANK_BACSREF, '' ) as SU_BANK_BACSREF,
