create view AA_SL_CREDITNOTE_SECURITY_VIEW
/*
** Written:			5/4/2004 RL
** Last Amended: 
** Comments: 		Returns Security Profile settings for Sales Credit Notes
*/
as
select
...
cast (substring(UP_TRAN_OPTIONS3, 7, 1) as tinyint) as BYPASS_COSTING,
(case 
   when substring(UP_TRAN_OPTIONS3, 8, 1) = 'Y' then cast(1 as tinyint)
   else cast(0 as tinyint)
   end)
as TERMS,
(case 
