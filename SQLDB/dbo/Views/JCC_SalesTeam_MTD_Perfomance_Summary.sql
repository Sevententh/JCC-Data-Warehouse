...
CREATE VIEW [dbo].[JCC_SalesTeam_MTD_Perfomance_Summary]

/*
** Written     :  11/09/2013 AKT
** Last Amended:  01/06/2016 AKT	
**
*/
...
WHEN [OH_SUB_LEDGER] = 'LWPROJECT' THEN 'Lucy Wilson'
WHEN [OH_SUB_LEDGER] = 'DTPROJECT' THEN 'Danielle Tuck'
WHEN [OH_SUB_LEDGER] = 'ARPROJECT' THEN 'Annelise Roberts'
WHEN [OH_SUB_LEDGER] = 'YHPROJECT' THEN 'Yazmin Hills'
ELSE ''
END
as ProjectManager
,[OD_ANALYSIS] as 'Nominal Account Code'
       ,'Dimensions Live DB' as DatabaseName
  FROM [JCC_PROD_LIVE].[dbo].[ORD_DETAIL] WITH (NOLOCK)
  LEFT OUTER JOIN [JCC_PROD_LIVE].[dbo].[ORD_HEADER] WITH (NOLOCK)
  ...
  LEFT OUTER JOIN [JCC_PROD_LIVE].[dbo].[SL_ACCOUNTS] WITH (NOLOCK)
  ON [OD_ACCOUNT] = [CUCODE]
  LEFT OUTER JOIN [JCC_DW_PRD_Presentation].[dbo].[DimSalesManager] WITH (NOLOCK)
  ON SUBSTRING([CUSORT], 1, 4) COLLATE DATABASE_DEFAULT = [SalesManagerCode] COLLATE DATABASE_DEFAULT
  WHERE CONVERT(VARCHAR(8),[OD_DATE],112) >= CONVERT(VARCHAR(8),dateadd(month,datediff(month,0,GETDATE())-1, 0),112)
  AND CONVERT(VARCHAR(8),[OD_DATE],112) < '20210522'
...
,[OD_ANALYSIS] as 'Nominal Account Code'
  ...
  FROM [JCC_PROJECTS].[dbo].[ORD_DETAIL] WITH (NOLOCK)
  LEFT OUTER JOIN [JCC_PROJECTS].[dbo].[ORD_HEADER] WITH (NOLOCK)
  ON [OD_ORDER_NUMBER] = [OH_ORDER_NUMBER]
  LEFT OUTER JOIN [JCC_DW_PRD_Presentation].[dbo].[DimProduct] WITH (NOLOCK)
  ON [OD_STOCK_CODE] COLLATE DATABASE_DEFAULT = [ProductCode] COLLATE DATABASE_DEFAULT
  LEFT OUTER JOIN [JCC_PROD_LIVE].[dbo].[SL_ACCOUNTS] WITH (NOLOCK)
