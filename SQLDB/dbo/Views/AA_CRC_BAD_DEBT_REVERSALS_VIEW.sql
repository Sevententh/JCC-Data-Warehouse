create view AA_CRC_BAD_DEBT_REVERSALS_VIEW
/*
** This View returns columns used in the Credit Control Bad Debts Written Off Report
** Written  :  26/9/07chw
** Amended  :  28/9/07chw,10/10/07chw, 16/10/07 NC, 17/10/07 NC, 25/10/07 NC, 17/12/07 NC
*/
...
,max(ReverseHeaderRef)              ReverseHeaderRef
...
,case when sign(max(InvoiceValue)) = -1 * sign(max(ST_GROSS)) and max(ST_TRANTYPE) = 'CRN' then 1
      when sign(max(InvoiceValue)) = 1  and max(ST_TRANTYPE) = 'ADR' then 1
      when sign(max(InvoiceValue)) = -1 and max(ST_TRANTYPE) = 'ACR' then 1
      else 0 end   Reverse
...
(
/* Get Invoices, Payments and Writeoffs*/
 select ih1.S_AL_REFERENCE          AllocReference
   ...
   ,rt.ST_HEADER_REF                ReverseHeaderRef
   ...
   from SL_TRANSACTIONS it
      ...
      inner join SL_ALLOC_HISTORY wh  with (nolock) on wh.S_AL_REFERENCE=ih2.S_AL_REFERENCE and wh.S_AL_REFERENCE<>'0'
      inner join SL_TRANSACTIONS wt   with (nolock) on wt.ST_HEADER_KEY=wh.S_AL_HEADER_KEY
      inner join SL_ALLOC_HISTORY wh2 with (nolock) on wt.ST_HEADER_KEY=wh2.S_AL_HEADER_KEY and wh2.S_AL_REFERENCE<>'0'
      inner join SL_ALLOC_HISTORY rh  with (nolock) on rh.S_AL_REFERENCE=wh2.S_AL_REFERENCE and rh.S_AL_REFERENCE<>'0'
      inner join SL_TRANSACTIONS rt   with (nolock) on rt.ST_HEADER_KEY=rh.S_AL_HEADER_KEY
   where ih1.S_AL_REFERENCE<>'0'
      and it.ST_WRITE_OFF_STATUS in (2,3)
      and it.ST_TRANTYPE='INV'
      and wt.ST_TRANTYPE<>'INV'
      and wt.ST_WRITE_OFF_STATUS = 2
      and rt.ST_TRANTYPE<>'INV'
      and rt.ST_WRITE_OFF_STATUS = 2
union all
/* Get Reverse writeoffs and Writeoffs*/
select rh.S_AL_REFERENCE            AllocReference
   ,it.ST_COPYCUST                  Customer
   ,it.ST_PRIMARY                   InvoicePrimary
   ,it.ST_YEAR                      InvoiceYear
   ,it.ST_PERIODNUMBER              InvoicePeriod
   ...
   from SL_TRANSACTIONS it
      ...
      inner join SL_ALLOC_HISTORY rh  with (nolock) on rh.S_AL_REFERENCE=wh2.S_AL_REFERENCE and rh.S_AL_REFERENCE<>'0'
      inner join SL_TRANSACTIONS rt   with (nolock) on rt.ST_HEADER_KEY=rh.S_AL_HEADER_KEY
   where it.ST_WRITE_OFF_STATUS in (2,3)
      and it.ST_TRANTYPE='INV'
      and wt.ST_TRANTYPE<>'INV'
      and wt.ST_WRITE_OFF_STATUS = 2
      and rt.ST_TRANTYPE<>'INV'
...
where (ST_TRANTYPE not in ('ADR','ACR') or ST_WRITE_OFF_STATUS>1) -- exclude adjustments unless they are bad debt adjustments
 and DET_ANALYSIS<>'' and isnull(ST_WRITE_OFF_STATUS,0)<>1
