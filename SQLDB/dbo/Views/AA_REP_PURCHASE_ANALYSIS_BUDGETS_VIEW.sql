Create View AA_REP_PURCHASE_ANALYSIS_BUDGETS_VIEW 
as
/*
** Written:     12/04/2005 RV/NC   
** Last Amended: 19/04/06 SRB
** Comments: returns purchase analysis budgets records for crystal reports
*/
...
(select isnull(R.B_REVISION_NUMBER,0) revision
   ...
   from ( select 1 as P_PERIOD union select 2 union select 3 union select 4 union select 5 union select 6 union select 7
      ...
      join PL_ANALYSIS S on S.PACODE <> ''
      left outer join SYS_BUDGETS R on R.B_ACCOUNT_CODE = S.PACODE and R.B_RECORD_TYPE = 'PA' and R.B_BUDGET_TYPE  = 'B'
      left outer join SYS_BUDGETS M on M.B_ACCOUNT_CODE = S.PACODE and M.B_RECORD_TYPE = 'PA' and M.B_BUDGET_TYPE  = 'B'
                                       and M.B_YEAR = Y.Y_YEAR and M.B_PERIOD = P.P_PERIOD
                                       and M.B_REVISION_NUMBER<=isnull(R.B_REVISION_NUMBER,0)
   group by S.PACODE,Y.Y_YEAR,P.P_PERIOD,isnull(R.B_REVISION_NUMBER,0)) L
