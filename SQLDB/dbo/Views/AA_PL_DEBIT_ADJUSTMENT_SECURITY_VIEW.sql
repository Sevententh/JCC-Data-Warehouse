create view AA_PL_DEBIT_ADJUSTMENT_SECURITY_VIEW
/*
** Written:			5/4/2004 RL
** Last Amended: 
** Comments: 		Returns Security Profile settings for Purchase Debit Adjustments (Decrease Supplier Balance)
*/
as
select
...
cast (substring(UP_TRAN_OPTIONS4, 7, 1) as tinyint) as REMEMBER_SUPPLIER_DETAILS,
cast (substring(UP_TRAN_OPTIONS4, 8, 1) as tinyint) as BYPASS_COSTING,
(case 
...
as ACCOUNT_NOTES,
(case 
   when substring(UP_TRAN_OPTIONS4, 12, 1) = 'Y' then cast(1 as tinyint)
   else cast(0 as tinyint)
   end)
as AGED_CREDITOR
