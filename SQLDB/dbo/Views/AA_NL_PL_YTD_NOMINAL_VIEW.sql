CREATE VIEW AA_NL_PL_YTD_NOMINAL_VIEW
AS
/*
** 
** Intended to be called by AA_NL_PROFIT_LOSS_NOMINAL_S with AA_NL_PROFIT_LOSS_NOMINAL_VIEW
** The main search fields are in AA_NL_PROFIT_LOSS_NOMINAL_VIEW along with the other financial fields.
...
**
** These fields are provided to identify a row in the main view (AA_NL_PROFIT_LOSS_NOMINAL_VIEW):
**          B_PERIOD
**          B_YEAR
**          CATEGORY
**          B_ACCOUNT_CODE
...
select 
   ...
   ,N.CATEGORY
   --£ Budget PTD
   ,dbo.AA_VALUE_DPS_F(isnull(BV.B_VALUE,cast(0 as decimal(24,8))) * case N.NMAJORHEADCODE when 1 then -1 when 3 then -1 when 10 then -1 when 11 then -1 when 12 then -1 else 1 end) BUD
   --£ Difference PTD
   ,dbo.AA_VALUE_DPS_F((case isnull(BV.B_VALUE,0) when 0 then cast(0 as decimal(24,8)) else N.NTURNOVER - BV.B_VALUE end) * (case N.NMAJORHEADCODE when 1 then -1 when 3 then -1 when 10 then -1 when 11 then -1 when 12 then -1 else 1 end)) DIFF
...
from 
   ...
   inner join
      (select N_PRIMARY [PRIMARY]
         ,case when CATEGORY = 0 then NL_MAJORCODE else cast(cast(NM_PRIMARY + 10 as nvarchar ) + '0' + 
            case 
