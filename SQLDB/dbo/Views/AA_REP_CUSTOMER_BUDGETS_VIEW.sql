Create View AA_REP_CUSTOMER_BUDGETS_VIEW 
as
/*
** Written:     23/03/2005 RV/NC   
** Last Amended: 20/09/05 RV , 19/04/06 SRB
** Comments: returns customer budgets records for crystal reports
*/
Select L.Customer as Customer
...
(select isnull(R.B_REVISION_NUMBER,0) revision
   ...
   from ( select 1 as P_PERIOD union select 2 union select 3 union select 4 union select 5 union select 6 union select 7
      ...
      join SL_ACCOUNTS C on C.CUCODE<>''
      left outer join SYS_BUDGETS R on R.B_ACCOUNT_CODE = C.CUCODE and R.B_RECORD_TYPE = 'S' and R.B_BUDGET_TYPE  = 'B' and R.B_YEAR = Y.Y_YEAR and R.B_PERIOD = P.P_PERIOD
      left outer join SYS_BUDGETS M on M.B_ACCOUNT_CODE = C.CUCODE and M.B_RECORD_TYPE = 'S' and M.B_BUDGET_TYPE  = 'B' and M.B_YEAR = Y.Y_YEAR and M.B_PERIOD = P.P_PERIOD
                                       and M.B_REVISION_NUMBER<=isnull(R.B_REVISION_NUMBER,0)
   group by C.CUCODE,Y.Y_YEAR,P.P_PERIOD,isnull(R.B_REVISION_NUMBER,0)) L
left outer join SYS_BUDGETS B on B.B_ACCOUNT_CODE = Customer and B.B_RECORD_TYPE = 'S' and B.B_BUDGET_TYPE  = 'B'
                                       and B.B_YEAR = Year and B.B_PERIOD = Period
                                       and B.B_REVISION_NUMBER=L.ActualRevision
group by L.Customer, L.revision