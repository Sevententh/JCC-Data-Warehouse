create view AA_SL_TRANSACTION_OPTIONS_VIEW
/*
** Written: 19/11/2004 SH
** Last Amended: 
** Comments: Returns SL transaction options specific to a user and a transaction type
*/
as

select O.UT_USER_ID as [USER_ID],
       case O.UT_TRAN_TYPE when 2 then 1320
                           when 3 then 1330
                           when 4 then 1340
                           when 5 then 1350
                           else 1310
       ...
       end as ALLOCATE_TRANSACTION_NOW,
       ...
       end as ENTER_GROSS_CURRENCY_AMOUNTS,
       ...
       end as ENTER_DUE_DATE_AND_RECEIPT_DATE,
...
where O.UT_MODULE = 'SL' and O.UT_TRAN_OPTION = 3