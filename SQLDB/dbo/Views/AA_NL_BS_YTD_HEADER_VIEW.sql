CREATE VIEW AA_NL_BS_YTD_HEADER_VIEW
AS
/*
** 
** Intended to be called by AA_NL_BALANCE_SHEET_HEADER_S with AA_NL_BALANCE_SHEET_HEADER_VIEW
** The main search fields are in AA_NL_BS_YTD_HEADER_VIEW along with the other financial fields.
...
select
   ...
   --Difference YTD %
   ,dbo.AA_VALUE_DPS_F(case isnull(B_VALUE,0) when 0 then cast(0 as decimal(24,8)) 
    else (NTURNOVER_YTD - B_VALUE) * (case NMAJORCODE when 1 then -1 when 3 then -1 when 10 then -1 when 11 then -1 when 12 then -1 else 1 end) end * (case B_VALUE * (case NMAJORCODE when 1 then -1 when 3 then -1 when 10 then -1 when 11 then -1 when 12 then -1 else 1 end) when 0 then 0 else 100/B_VALUE * (case NMAJORCODE when 1 then -1 when 3 then -1 when 10 then -1 when 11 then -1 when 12 then -1 else 1 end) end)) DIFF_YTD_PC
   --£ Last Year Budget YTD
   ,dbo.AA_VALUE_DPS_F(BUD_YTD_L) BUD_YTD_L
   ,isnull(BV.B_ACCOUNT_CODE,N.NCODE) B_ACCOUNT_CODE
   ,isnull(BV.B_REVISION_NUMBER,0) REV_NO
from 
   AA_NL_BALANCE_SHEET_HEADER_VIEW N 
   left join SYS_BUDGETS BV on
      ...
      from 
         ...
         left join SYS_BUDGETS BV on
            ...
            and case N.YEAR when 'L' then -1 when 'C' then 0 else 1 end = BV.B_YEAR
      where
         isnull(BV.B_RECORD_TYPE,'N')     = 'N'
         and   BV.B_YEAR                  = -1
         and isnull(BV.B_BUDGET_TYPE,'B') = 'B'
      group by N.[PRIMARY]
         ,BV.B_YEAR 
         ,N.YEAR
         ,BV.B_PERIOD 
